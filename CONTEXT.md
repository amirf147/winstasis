...
* **Omniscient Workspace Tracking:** `winstasis` orchestrates windows across *all* active Virtual Desktops globally. It relies on the `Slions.VirtualDesktop` package to safely wrap the undocumented `IVirtualDesktopManagerInternal` COM interface, bypassing OS security restrictions.
* **Pinned Windows:** Applications configured via Windows Task View to "Show this window on all desktops." `winstasis` detects and re-applies this state during restoration.
* **UIPI (User Interface Privilege Isolation):** A core Windows security feature that prevents a standard process (`winstasis`) from moving or modifying a window owned by an Administrator process. To restore Admin windows, `winstasis` must be run as Administrator.
