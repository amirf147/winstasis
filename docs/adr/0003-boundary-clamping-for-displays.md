...
## Decision
We will use **Boundary Clamping**. During a restore operation, `winst` will check the target coordinates against the bounds of the currently active display(s) using the Windows API (`MonitorFromRect` / `GetMonitorInfo`). If the target rectangle is completely outside the visible desktop area, `winst` will clamp its X and Y coordinates to the nearest visible edge before applying the move.
