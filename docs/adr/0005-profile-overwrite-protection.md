...
## Context
When users run the save command (e.g., `winst save coding`), there is a high likelihood they might accidentally type the name of an existing profile, destroying a carefully crafted window layout they spent time arranging.

## Decision
We will implement a **Safety Prompt** system for profile saves. 
If a profile with the requested name already exists in the `sessions/` directory, `winst` will halt execution and prompt the user in the CLI: `"Profile '[name]' already exists. Overwrite? (y/n)"`. 
